...
if command -v ntn >/dev/null 2>&1; then
  ...
  if confirm "Install @notionhq/cli globally now?" "Y"; then
    npm i -g @notionhq/cli
    ok "ntn installed"
  else
    info "You can run it later with: npx -y @notionhq/cli <args>"
  fi
fi
...
prompt_token "DISCORD_BOT_TOKEN"  "Discord bot token (only needed for Discord syncs/tools)"  "https://discord.com/developers/applications"
...
if ! grep -qE '^KANBAN_GIST_URL=' "$ENV_FILE" 2>/dev/null; then
  write_env_var "KANBAN_GIST_URL" ""
fi
...
${BOLD}Next steps:${RESET}
  ...
  ${DIM}# 1. Smoke-test locally (no deploy):${RESET}
  ntn workers exec sayHello -d '{"name":"world"}' --local

  ${DIM}# 2. Trigger your first sync locally:${RESET}
  ntn workers exec projectsFromDiscord --local        ${DIM}# needs DISCORD_BOT_TOKEN${RESET}

  ${DIM}# 3. Deploy to Notion cloud (first run links the worker):${RESET}
  ntn workers deploy

  ${DIM}# 4. Watch runs:${RESET}
  ntn workers runs list --limit 10

${DIM}Docs:${RESET}  README.md  ·  AGENTS.md  ·  docs/CHECKPOINT.md
