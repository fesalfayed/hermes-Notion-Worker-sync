...
| **4.4** | Drift watchdog → row-level diffs | Watchdog promoted from a count comparator to a 4-category row-level differ (kanban-only / notion-only / status-mismatch / orphan-relations) → `local/state/drift_latest.json`. Daily digest cron at `0 9 * * *` posts to `#daily-updates` (`1503874454584037406`), silent-on-zero. |
...
- "Archive the project bound to channel 1504266202506199272."
