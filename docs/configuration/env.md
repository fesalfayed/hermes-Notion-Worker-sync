...
| `AGENTIC_GUILD_ID` | `scripts/seed-board-map.ts` only | optional | Discord guild ID (right-click server in Discord with Developer Mode on) | Not used by the runtime worker. The runtime `GUILD_ID` is hardcoded in `src/constants.ts`. |
...
- **`scripts/seed-board-map.ts`** — only reads `AGENTIC_GUILD_ID` and `DISCORD_BOT_TOKEN`; it's a local helper that emits YAML to stdout and never touches Notion.
