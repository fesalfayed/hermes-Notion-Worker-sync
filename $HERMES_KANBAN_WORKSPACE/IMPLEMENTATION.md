...
**File**: `/Users/fesal/hermes-projects-sync/src/index.ts` (lines 82-156)
...
worker.tool("archiveProject", {
  ...
  execute: async (input) => {
    ...
    // 6. Return { ok: true, from_category, to_category: "1504372848708554793" }
  }
})
```
...
- ✓ Execute: `PATCH /channels/<id>` with `{ parent_id: "1504372848708554793" }`
- ✓ Return: `{ok, from_category, to_category:"1504372848708554793"}`
...
**File**: `/Users/fesal/hermes-projects-sync/src/index.ts` (lines 158-232)
...
worker.tool("unarchiveProject", {
  ...
  execute: async (input) => {
    ...
    // 6. Return { ok: true, from_category: "1504372848708554793", to_category }
  }
})
```
...
- ✓ Schema: `j.object({ discord_channel_id: j.string() })`
- ✓ Execute: `PATCH /channels/<id>` with `{ parent_id: "1503576075723079700" }`
- ✓ Return: `{ok, from_category:"1504372848708554793", to_category}`
...
```typescript
const PROJECTS_CATEGORY_ID = "1503576075723079700";
const ARCHIVE_CATEGORY_ID = "1504372848708554793";
```
...
```bash
$ cd /Users/fesal/hermes-projects-sync && npm run build
...
2. **Test Archive**: Execute `ntn workers exec archiveProject --input '{"discord_channel_id": "1504560480671498260"}'`
3. **Verify Notion**: Wait for sync tick (~5m) or force via `ntn workers exec projectsFromDiscord`
4. **Check state**: Notion row should show `discord_archived=true`, `discord_category_id=1504372848708554793`
5. **Test Unarchive**: Execute `ntn workers exec unarchiveProject --input '{"discord_channel_id": "1504560480671498260"}'`
6. **Re-verify**: Notion row should show `discord_archived=false`, `discord_category_id=1503576075723079700`
